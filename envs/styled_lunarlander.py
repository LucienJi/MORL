...
LunarLander_Factors = {
    'shaping_factor': {"lower_bound": 1, "upper_bound": 1, "fine_grit": 0.1},
    'main_engine_factor':{"lower_bound": 1, "upper_bound": 1, "fine_grit": 0.5},
    "side_engine_factor":{"lower_bound": 1, "upper_bound": 1, "fine_grit": 0.1},
}
...
class StyledLunarLander(gym.Env, EzPickle):
    """
    ### Description
    ...
    def __init__(
        ...
        if reward_factors is not None:
            self.reward_factors = reward_factors
        else:
            self.reward_factors = {
                'shaping_factor': 1.0,
                'main_engine_factor':1.0,
                "side_engine_factor":1.0
            }
    ...
    def reset(
        self,
        new_factor:dict = None
    ):  
        if new_factor is not None:
            for k,v in new_factor.items():
                if k in self.reward_factors.keys():
                    self.reward_factors[k] = v
        self._destroy()
    ...
    def step(self, action):
        ...
        if self.prev_shaping is not None:
            ...
            shaping_reward = 0
        main_engine_reward = -m_power * 0.3 
        side_engine_reward = -s_power * 0.03 
        self.prev_shaping = shaping

        reward += (
            self.reward_factors['shaping_factor'] * shaping_reward
            + self.reward_factors['main_engine_factor'] * main_engine_reward
            + self.reward_factors['side_engine_factor'] * side_engine_reward
        )
